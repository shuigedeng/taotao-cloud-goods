...
import com.taotao.cloud.goods.application.flow.flow.treeflow.channel.ManualConfirmType;
...
import com.taotao.cloud.goods.application.flow.flow.treeflow.flow.nodes.FlowTree;
import com.taotao.cloud.goods.application.flow.flow.treeflow.flow.nodes.VirtualFlow;
...
import static com.alibaba.csp.sentinel.util.AssertUtil.assertNotNull;
...
public class FlowCenter {
	...
	public String flowHandlerInfo() {
		if (null != flowHandlerTable && !flowHandlerTable.isEmpty()) {
	...
	public String flowBusinessHandlerInfo() {
		if (null != businessHandlerTable && !businessHandlerTable.isEmpty()) {
			StringBuilder sb = new StringBuilder();
	...
	public FlowCenter(FlowRegister register, FlowEventRecordInfoInterface flowEventRecordInfoInterface,
		...
		assertNotNull(register, "FlowCenter.FlowCenter,注册中心为null");
		assertNotNull(flowEventRecordInfoInterface, "FlowCenter.FlowCenter,事件记录服务为null");
	...
	private void doFlow(Flow flowTree, FlowContext flowContext, FlowEventRecordInfo updateFlowEventRecord) {
		...
		if (null != flows && !flows.isEmpty()) {
			...
			// flowEventL
			if (null != noCompleteL && !noCompleteL.isEmpty()) {
	...
	private void parse(Flow flow, FlowContext flowContext) {
		...
		if (null == nextFlow) {
			if (isEnd) {
				...
				while (null != currentEventRecordInfo && !StringUtils.isEmpty(currentEventRecordInfo.getParentRecordId())) {
