...
import com.taotao.cloud.goods.application.flow.flow.callback.*;
	...
	public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
		...
		if (gatewayResCallbackConsumerMap.size() > 0) {
			gatewayResCallbackConsumerMap.forEach((k, v) -> {
				CallbackFlowCenterInst gatewayCallbackInst = v.getClass().getAnnotation(CallbackFlowCenterInst.class);
				...
				for (Method declaredMethod : declaredMethods) {
					...
					if (name.equals("extract")) {
						...
						if (1 == parameterTypes.length) {
							installFlowTraceIdExtract(parameterTypes[0].getClass(), v);
						}
					}
				}
			});
