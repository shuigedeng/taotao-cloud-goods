...
import com.taotao.cloud.goods.application.flow.flow.treeflow.flow.nodes.*;
import com.taotao.cloud.goods.application.flow.flow.treeflow.info.FlowInfo;
import com.taotao.cloud.goods.application.flow.flow.treeflow.info.FlowRegisterInfo;
import com.taotao.cloud.goods.application.flow.flow.treeflow.info.FlowTreeFindInfo;
...
// 业务描述-业务编码
	...
	public FlowTreeBuilder flowTreeRoot( FlowInfo root ) {
		...
		String nameDesc = root.getNameDesc();
		...
		if (flow instanceof VirtualFlow) {
			...
			if (null != flowTreeList && !flowTreeList.isEmpty()) {
				...
				for (FlowTree.PredictConditionFlow predictConditionFlow : flowTreeList) {
					Flow predictConditionFlowFlow = predictConditionFlow.getFlow();
					FlowTreeDesc childrenFlowTreeDesc = buildFlowTreeDesc(predictConditionFlowFlow);
	...
	public class FlowTreeBuilder {
		...
		public FlowTreeBuilder processComplete() {
			return this.condition(flowContext -> flowContext.getFlowEventRecordInfo()
					.getStatus().equals(BusinessFlowStatus.P))
				.flowComplete();
		}
		...
		public FlowTreeBuilder parallelFlowTree( FlowInfo flowInfo, boolean async ) {
			// 流程节点描述-名称
			String name = flowInfo.getName();
			String nameDesc = flowInfo.getNameDesc();  // 修正：getUserNameDesc() → getNodeDesc()
			if (StringUtils.isEmpty(nameDesc)) {
				nameDesc = name;
			}
		...
		public FlowTreeBuilder successFlowTree( FlowInfo flowInfo, boolean async ) {
			// 流程节点描述-名称
			String name = flowInfo.getName();
			String nameDesc = flowInfo.getNameDesc();  // 修正
			if (StringUtils.isEmpty(nameDesc)) {
				nameDesc = name;
			}
		...
		public FlowTreeBuilder virtualFlow( FlowInfo flowInfo, String type, boolean async ) {
			// 流程节点描述-名称
			String name = flowInfo.getName();
			String nameDesc = flowInfo.getNameDesc();  // 修正：使用正确的描述字段
			if (StringUtils.isEmpty(nameDesc)) {
				nameDesc = name;
			}
		...
		public FlowTreeConditionBuilder success( Predicate<FlowContext> predicate, String desc ) {
			...
			return condition(flowContext -> flowContext.eventIsSuccess() && predicate.test(flowContext),
			"流程成功且" + desc);
		}
		...
		public FlowTreeBuilder build() {
			...
			if (this.flowTree instanceof ParallelFlowTree) {
				for (FlowTreeParallelBuilder flowTreeParallelBuilder : flowTreeParallelBuilderList) {
					( (ParallelFlowTree) this.flowTree ).parallel(flowTreeParallelBuilder.flowTree);
				}
			}
			...
			for (FlowTreeConditionBuilder flowTreeConditionBuilder : flowTreeConditionBuilderList) {
				...
				if (!StringUtils.isEmpty(predictDesc)) {
					flowTree.grow(flowTreeConditionBuilder.getPredicate(), predictDesc,
						flowTreeConditionBuilder.flowTree);
				} else {
					flowTree.grow(flowTreeConditionBuilder.getPredicate(), flowTreeConditionBuilder.flowTree);
				}
			}
		...
		public FlowTreeBuilder backTo( FlowInfo flowInfo ) {
			...
			FlowTree upFlowTree = upFlowTreeBuilder.flowTree;  // 使用 getter
			if (null == upFlowTree) {
				throw new IllegalArgumentException(
