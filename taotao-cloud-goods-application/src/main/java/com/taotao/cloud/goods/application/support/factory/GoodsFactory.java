...
package com.taotao.cloud.goods.application.support.factory;
...
public class GoodsFactory implements MarkerFactory {
	...
	public static GoodsAgg createGoods( GoodsCreateCommand goodsCreateCommand ) {
		return new GoodsAgg(
			BizId.newBizId(),
			BizId.fromValue(goodsCreateCommand.categoryId()),
			GoodsName.of(goodsCreateCommand.goodsName()),
			GoodsSpecBuilder
				.builder()
				.manufactureDate(goodsCreateCommand.manufactureDate())
				.expirationDate(goodsCreateCommand.expirationDate())
				.goodsWeight(GoodsWeightBuilder
					.builder()
					.weight(goodsCreateCommand.goodsWeight())
					.unit(WeightUnit.of(goodsCreateCommand.goodsWeightUnit()))
					.build())
				.goodsDesc(goodsCreateCommand.goodsDesc())
				.build(),
			Price.of(goodsCreateCommand.goodsPrice()),
			GoodsStatus.UNSHELVED,
			BizId.fromNullableValues(goodsCreateCommand.tagIds()));
	}
}
