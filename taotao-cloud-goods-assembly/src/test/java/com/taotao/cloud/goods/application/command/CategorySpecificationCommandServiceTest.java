...
/**
 * CategorySpecificationCommandService 集成测试
 *
 * @author shuigedeng
 * @version 2026.04
 * @since 2026-08-22
 */
class CategorySpecificationCommandServiceTest extends TaoTaoCloudGoodsApplicationTests {
    ...
    class DeleteByCategoryId {
        ...
        void shouldReturnFalseForNonexistentCategory() {
            boolean result = categorySpecificationCommandService.deleteByCategoryId(99999L);
            assertThat(result).isFalse();
        }

        @Test
        void shouldReturnFalseForNullCategoryId() {
            boolean result = categorySpecificationCommandService.deleteByCategoryId(null);
            assertThat(result).isFalse();
        }
    }
}
