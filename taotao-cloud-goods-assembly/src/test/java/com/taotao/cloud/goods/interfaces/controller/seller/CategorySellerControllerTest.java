...
@WebMvcTest(CategorySellerController.class)
public class CategorySellerControllerTest {
	...
	@Nested
	class QueryCategoryAll {

		@Test
		void shouldReturnNull() throws Exception {
			mockMvc.perform(get("/seller/goods/category/store/query/category/all"))
				.andExpect(status().isOk());
		}
	}

	@Nested
	class QueryBrandsByCategoryId {

		@Test
		void shouldReturnBrandsWhenCategoryExists() throws Exception {
			...
			mockMvc.perform(get("/seller/goods/category/store/query/category-id/brands")
					.param("categoryId", "1"))
				.andExpect(status().isOk())
